...
import json
for chain, obj in OLDchainAPIs.items():
    
    v = {
            "denom": "",
            "name": "",
            "coingecko_id": "",
            "explorers": {name: link.rsplit("/", 1)[0] for name, link in obj[1].items()},
            "rest_root": f"{obj[0].split('/cosmos/')[0]}", 
            "rpc_root": "",
            "twitter": f"{obj[-1]}",
            "logo": "",
            "chain-registry": "",
    ...
    # print above with indent of 4
    print(f"'{chain}':", json.dumps(v, indent=4), end=",\n")
